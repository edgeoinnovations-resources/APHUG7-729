...
function initAutoSave(chapterId) {
  ...
  for (let dok = 1; dok <= 4; dok++) {
    if (localStorage.getItem(`${chapterId}_dok${dok}_done`) === 'true') {
      const btn = document.querySelector(`[id*="dok${dok}"], [id*="done${dok}"], [id="dok${dok}Btn"]`);
...
function gatherMCQScore() {
  let correct = 0;
  let total = 0;
  // Handle both naming conventions across files
  document.querySelectorAll('.mcq-item').forEach(item => {
    if (!item.dataset.answered && !item.querySelector('.mcq-choice.correct, .mcq-choice.incorrect, .mcq-btn.correct, .mcq-btn.incorrect')) return;
    total++;
    const hasIncorrect = item.querySelector('.mcq-choice.incorrect, .mcq-btn.incorrect');
...
function gatherTextareaResponses(chapterId) {
  ...
  document.querySelectorAll('textarea, .sa-textarea').forEach((ta, i) => {
    ...
    if (val) {
      // Try to find a nearby label/prompt
      const card = ta.closest('.card');
      const label = card?.querySelector('.card-label, h3, h4')?.textContent?.trim() || `Response ${i + 1}`;
...
// Call this to inject the completion section at the bottom of any chapter
function buildCompletionSection(chapterId, chapterTitle) {
  ...
  section.style.display = 'none'; // Hidden until all DOKs complete
  section.innerHTML = `
...
function buildStudentNameBar() {
  ...
  // Insert right after progress bar, or at the top of body
...
function updateHomePageStatus() {
  const chapters = ['7_0', '7_1', '7_2', '7_3', '7_4', '7_5', '7_6', '7_7', '7_8', '7_9'];
  chapters.forEach(ch => {
